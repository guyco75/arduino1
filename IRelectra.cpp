...
// 3. Send to IRremote
bool IRelectra::SendElectra(int power, int mode, int fan, int temperature, int swing, int sleep, bool tempNotif, bool iFeel)
{
    unsigned int data[200]; //~maximum size of the IR packet
    int i = 0;
 
    // get the data representing the configuration
    uint64_t code = EncodeElectra(power, mode, fan, temperature, swing, sleep, tempNotif, iFeel);
